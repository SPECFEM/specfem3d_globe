...
#ifdef USE_TEXTURES_FIELDS\n\
    s_dummy_loc[tx] = as_float(read_imageui(d_b_displ_oc_tex, sampler_d_b_displ_oc_tex, int2(iglob,0)).x);\n\
#else\n\
...
#ifdef USE_MESH_COLORING_GPU\n\
#ifdef USE_TEXTURES_FIELDS\n\
    d_potential_dot_dot[iglob] = as_float(read_imageui(d_b_accel_oc_tex, sampler_d_b_accel_oc_tex, int2(iglob,0)).x) + sum_terms;\n\
#else\n\
