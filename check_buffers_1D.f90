...
! we add one to the size of the array for the final flag
  integer, dimension(:), allocatable :: iboolleft,iboolright,icodeleft,icoderight
...
! dynamic memory allocation for arrays
  ...
  allocate(icodeleft(NPOIN1D_RADIAL(iregion_code)+1))
  allocate(icoderight(NPOIN1D_RADIAL(iregion_code)+1))
...
! read ibool1D_rightxi_righteta of this slice
  ...
  npoin1D = 1
 360  continue
  read(34,*) iboolright(npoin1D),icoderight(npoin1D),xright(npoin1D),yright(npoin1D),zright(npoin1D)
...
! read ibool1D_leftxi_righteta of other slice
  ...
  npoin1D = 1
 350  continue
  read(34,*) iboolleft(npoin1D),icodeleft(npoin1D),xleft(npoin1D),yleft(npoin1D),zleft(npoin1D)
...
! check codes
      if(icodeleft(ipoin) /= icoderight(ipoin)) then
        print *,'error: different codes along xi ',icodeleft(ipoin),icoderight(ipoin),' in region ',iregion_code
...
! check codes
      if(icodeleft(ipoin) /= icoderight(ipoin)) then
        print *,'error: different codes along eta ',icodeleft(ipoin),icoderight(ipoin),' in region ',iregion_code
        stop 'error: different codes'
      endif
  enddo
...
! deallocate arrays
  ...
  deallocate(icodeleft)
  deallocate(icoderight)
  deallocate(xleft)
  deallocate(yleft)
  deallocate(zleft)
  deallocate(xright)
