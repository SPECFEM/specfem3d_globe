...
!=====================================================================
  ...
  subroutine compute_forces_inner_core(minus_gravity_table,density_table,minus_deriv_gravity_table, &
          iter,displ,accel,xstore,ystore,zstore, &
          ...
          one_minus_sum_beta,alphaval,betaval,gammaval,factor_common,update_dof)
...
! global points in the matching regions
  logical, dimension(NGLOB_INNER_CORE) :: update_dof
...
! define flag to selectively update elements at second iteration
    if(idoubling(ispec) == IFLAG_TOP_INNER_CORE &
        .or. idoubling(ispec) == IFLAG_TOP_INNER_CORE_LEV2) then
...
! sum contributions from each element to the global mesh and add gravity terms
    do k=1,NGLLZ
      do j=1,NGLLY
        do i=1,NGLLX
          iglob = ibool(i,j,k,ispec)
          if(iter == 1 .or. update_dof(iglob)) accel(:,iglob) = accel(:,iglob) + sum_terms(:,i,j,k)
        enddo
      enddo
    enddo

! use Runge-Kutta scheme to march memory variables in time
! update at first iteration everywhere except in the matching layer at the ICB
