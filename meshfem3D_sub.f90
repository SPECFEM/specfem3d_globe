...
! parameters derived from input parameters
  integer MIN_ATTENUATION_PERIOD,MAX_ATTENUATION_PERIOD, &
             ...
             NER_ICB_CMB,NER_DOUBLING_OUTER_CORE, &
             NEX_ETA,NEX_XI, &
             NPROC_ETA,NPROC_XI,NSTEP
...
! do not create anything for the inner core here, will be done in solver
  ...
  call MPI_COMM_SIZE(MESHER_COMM,sizeprocs,ier)
  call MPI_COMM_RANK(MESHER_COMM,myrank,ier)
...
! derive parameters from input parameters
  call derive_parameters(myrank,TMIN, RECORD_LENGTH, sizeprocs, &
...
! make sure everybody is synchronized
  call MPI_BARRIER(MESHER_COMM,ier)
