...
! matching codes
  integer, dimension(:,:), allocatable :: icodefaces_crust_mantle
  integer, dimension(:,:), allocatable :: icodefaces_outer_core
  integer, dimension(:,:), allocatable :: icodefaces_inner_core
...
! global points in the matching regions
  logical, dimension(NGLOBMAX_CRUST_MANTLE) :: update_dof_crust_mantle
...
! displacement, velocity, acceleration
  real(kind=CUSTOM_REAL), dimension(NDIM,NGLOBMAX_CRUST_MANTLE) :: &
     displ_crust_mantle,veloc_crust_mantle,vpred_crust_mantle,accel_crust_mantle
...
! global points in the matching regions
  logical, dimension(NGLOBMAX_OUTER_CORE) :: update_dof_outer_core
...
! velocity potential
  real(kind=CUSTOM_REAL), dimension(NGLOBMAX_OUTER_CORE) :: displ_outer_core, &
    veloc_outer_core,vpred_outer_core,accel_outer_core
...
! global points in the matching regions
  logical, dimension(NGLOB_INNER_CORE) :: update_dof_inner_core
...
! displacement, velocity, acceleration
  real(kind=CUSTOM_REAL), dimension(NDIM,NGLOB_INNER_CORE) :: &
     displ_inner_core,veloc_inner_core,vpred_inner_core,accel_inner_core
...
! iterations for fluid-solid matching
  integer iter
...
! matching codes
  integer, dimension(:,:), allocatable :: icodecorner_crust_mantle,icodecorner_outer_core,icodecorner_inner_core
...
! codes
  integer, dimension(:), allocatable :: icode_xi_crust_mantle,icode_eta_crust_mantle
  integer, dimension(:), allocatable :: icode_xi_outer_core,icode_eta_outer_core
  integer, dimension(:), allocatable :: icode_xi_inner_core,icode_eta_inner_core
...
! only simple iterative scheme implemented
  if(NITER > 2) call exit_MPI(myrank,'only one or two iterations implemented')
  if((ACTUALLY_COUPLE_FLUID_CMB .or. ACTUALLY_COUPLE_FLUID_ICB) .and. NITER == 1) &
    call exit_MPI(myrank,'need more than one iteration for fluid/solid coupling')
...
! matching codes
! maximum size is found in the mantle which has the largest number of points
  allocate(icodecorner_crust_mantle(NPOIN1D_RADIAL(IREGION_CRUST_MANTLE),NUMCORNERS_SHARED))
  allocate(icodecorner_outer_core(NPOIN1D_RADIAL(IREGION_OUTER_CORE),NUMCORNERS_SHARED))
  allocate(icodecorner_inner_core(NPOIN1D_RADIAL(IREGION_INNER_CORE),NUMCORNERS_SHARED))
...
! crust and mantle
  ...
  allocate(icode_xi_crust_mantle(NPOIN2DMAX_XMIN_XMAX(IREGION_CRUST_MANTLE)))
  allocate(icode_eta_crust_mantle(NPOIN2DMAX_YMIN_YMAX(IREGION_CRUST_MANTLE)))
...
! outer core
  ...
  allocate(icode_xi_outer_core(NPOIN2DMAX_XMIN_XMAX(IREGION_OUTER_CORE)))
  allocate(icode_eta_outer_core(NPOIN2DMAX_YMIN_YMAX(IREGION_OUTER_CORE)))
...
! inner core
  ...
  allocate(icode_xi_inner_core(NPOIN2DMAX_XMIN_XMAX(IREGION_INNER_CORE)))
  allocate(icode_eta_inner_core(NPOIN2DMAX_YMIN_YMAX(IREGION_INNER_CORE)))
...
! allocate arrays for message buffers with maximum size
  ...
  allocate(icodefaces_crust_mantle(NPOIN2DMAX_XY,NUMFACES_SHARED))
  allocate(icodefaces_outer_core(NPOIN2DMAX_XY,NUMFACES_SHARED))
  allocate(icodefaces_inner_core(NPOIN2DMAX_XY,NUMFACES_SHARED))
...
! nothing to create in the crust (no coupling layer there)

! initialize arrays
  update_dof_crust_mantle(:) = .false.
  update_dof_outer_core(:) = .false.
  update_dof_inner_core(:) = .false.

! mantle and crust (bottom = CMB, no top coupling layer)
  call define_update_dof(update_dof_crust_mantle,ibool_crust_mantle,idoubling_crust_mantle, &
        nspec_crust_mantle,nglob_crust_mantle, &
        IFLAG_BOTTOM_MANTLE,IFLAG_BOTTOM_MANTLE_LEV2,IFLAG_DUMMY,IFLAG_DUMMY)

! outer core (bottom = ICB, top = CMB)
  call define_update_dof(update_dof_outer_core,ibool_outer_core,idoubling_outer_core, &
          nspec_outer_core,nglob_outer_core, &
          IFLAG_BOTTOM_OUTER_CORE,IFLAG_BOTTOM_OUTER_CORE_LEV2,IFLAG_TOP_OUTER_CORE,IFLAG_TOP_OUTER_CORE_LEV2)

! inner core (no bottom coupling layer, top = ICB)
  call define_update_dof(update_dof_inner_core,ibool_inner_core,idoubling_inner_core, &
          NSPEC_INNER_CORE,NGLOB_INNER_CORE, &
          IFLAG_DUMMY,IFLAG_DUMMY,IFLAG_TOP_INNER_CORE,IFLAG_TOP_INNER_CORE_LEV2)
...
! mantle and crust
  call read_arrays_buffers_solver(IREGION_CRUST_MANTLE,myrank,iboolleft_xi_crust_mantle, &
     iboolright_xi_crust_mantle,iboolleft_eta_crust_mantle,iboolright_eta_crust_mantle, &
     icode_xi_crust_mantle,icode_eta_crust_mantle,npoin2D_xi_crust_mantle,npoin2D_eta_crust_mantle, &
     iprocfrom_faces,iprocto_faces,imsg_type, &
     iproc_master_corners,iproc_slave1_corners,iproc_slave2_corners, &
     iboolfaces_crust_mantle,icodefaces_crust_mantle,npoin2D_faces_crust_mantle, &
     iboolcorner_crust_mantle,icodecorner_crust_mantle, &
...
! outer core
  call read_arrays_buffers_solver(IREGION_OUTER_CORE,myrank, &
     ...
     icode_xi_outer_core,icode_eta_outer_core,npoin2D_xi_outer_core,npoin2D_eta_outer_core, &
     iprocfrom_faces,iprocto_faces,imsg_type, &
     iproc_master_corners,iproc_slave1_corners,iproc_slave2_corners, &
     iboolfaces_outer_core,icodefaces_outer_core,npoin2D_faces_outer_core, &
     iboolcorner_outer_core,icodecorner_outer_core, &
...
! inner core
  call read_arrays_buffers_solver(IREGION_INNER_CORE,myrank, &
     ...
     icode_xi_inner_core,icode_eta_inner_core,npoin2D_xi_inner_core,npoin2D_eta_inner_core, &
     iprocfrom_faces,iprocto_faces,imsg_type, &
     iproc_master_corners,iproc_slave1_corners,iproc_slave2_corners, &
     iboolfaces_inner_core,icodefaces_inner_core,npoin2D_faces_inner_core, &
     iboolcorner_inner_core,icodecorner_inner_core, &
...
! set iter to 1 to assemble mass matrix since used by assembling routine
  iter = 1

! ocean load
  call assemble_MPI_scalar(myrank,iter,rmass_ocean_load,nglob_crust_mantle, &
            iproc_xi,iproc_eta,ichunk,addressing, &
            iboolleft_xi_crust_mantle,iboolright_xi_crust_mantle,iboolleft_eta_crust_mantle,iboolright_eta_crust_mantle, &
            npoin2D_faces_crust_mantle,npoin2D_xi_crust_mantle,npoin2D_eta_crust_mantle, &
            iboolfaces_crust_mantle,iboolcorner_crust_mantle, &
            icodefaces_crust_mantle,icode_xi_crust_mantle,icode_eta_crust_mantle,icodecorner_crust_mantle, &
            iprocfrom_faces,iprocto_faces,imsg_type, &
            iproc_master_corners,iproc_slave1_corners,iproc_slave2_corners, &
...
! crust and mantle
  call assemble_MPI_scalar(myrank,iter,rmass_crust_mantle,nglob_crust_mantle, &
            iproc_xi,iproc_eta,ichunk,addressing, &
            iboolleft_xi_crust_mantle,iboolright_xi_crust_mantle,iboolleft_eta_crust_mantle,iboolright_eta_crust_mantle, &
            npoin2D_faces_crust_mantle,npoin2D_xi_crust_mantle,npoin2D_eta_crust_mantle, &
            iboolfaces_crust_mantle,iboolcorner_crust_mantle, &
...
! outer core
  call assemble_MPI_scalar(myrank,iter,rmass_outer_core,nglob_outer_core, &
            iproc_xi,iproc_eta,ichunk,addressing, &
            iboolleft_xi_outer_core,iboolright_xi_outer_core,iboolleft_eta_outer_core,iboolright_eta_outer_core, &
            npoin2D_faces_outer_core,npoin2D_xi_outer_core,npoin2D_eta_outer_core, &
            iboolfaces_outer_core,iboolcorner_outer_core, &
            icodefaces_outer_core,icode_xi_outer_core,icode_eta_outer_core,icodecorner_outer_core, &
            iprocfrom_faces,iprocto_faces,imsg_type, &
            iproc_master_corners,iproc_slave1_corners,iproc_slave2_corners, &
            buffer_send_faces,buffer_received_faces, &
            buffer_send_chunk_corners,buffer_received_chunk_corners, &
...
! inner core
  call assemble_MPI_scalar(myrank,iter,rmass_inner_core,NGLOB_INNER_CORE, &
            iproc_xi,iproc_eta,ichunk,addressing, &
            iboolleft_xi_inner_core,iboolright_xi_inner_core,iboolleft_eta_inner_core,iboolright_eta_inner_core, &
            npoin2D_faces_inner_core,npoin2D_xi_inner_core,npoin2D_eta_inner_core, &
            iboolfaces_inner_core,iboolcorner_inner_core, &
            icodefaces_inner_core,icode_xi_inner_core,icode_eta_inner_core,icodecorner_inner_core, &
            iprocfrom_faces,iprocto_faces,imsg_type, &
            iproc_master_corners,iproc_slave1_corners,iproc_slave2_corners, &
            buffer_send_faces,buffer_received_faces, &
            buffer_send_chunk_corners,buffer_received_chunk_corners, &
...
! rescale in crust and mantle
  ...
  if(idoubling_crust_mantle(ispec) == IFLAG_DOUBLING_670 .or. &
     ...
     idoubling_crust_mantle(ispec) == IFLAG_BOTTOM_MANTLE_LEV2 .or. &
...
!
  ...
  if(myrank == 0) then
    ...
    write(IMAIN,*) 'number of iterations NITER = ',NITER
    write(IMAIN,*)
  endif
...
!CDIR NOASSUME
  do i=1,nglob_crust_mantle_ndim
    ...
    vpred_crust_mantle(i,1) = veloc_crust_mantle(i,1) + deltatover2*accel_crust_mantle(i,1)
    veloc_crust_mantle(i,1) = vpred_crust_mantle(i,1)
  enddo

! outer core
  do i=1,nglob_outer_core
    displ_outer_core(i) = displ_outer_core(i) + deltat*veloc_outer_core(i) + deltatsqover2*accel_outer_core(i)
    vpred_outer_core(i) = veloc_outer_core(i) + deltatover2*accel_outer_core(i)
    veloc_outer_core(i) = vpred_outer_core(i)
  enddo
...
!CDIR NOASSUME
  do i=1,NGLOB_INNER_CORE*NDIM
    ...
    vpred_inner_core(i,1) = veloc_inner_core(i,1) + deltatover2*accel_inner_core(i,1)
    veloc_inner_core(i,1) = vpred_inner_core(i,1)
  enddo
...
! ***********************************
!   iterate on fluid/solid matching
! ***********************************
...
! for anisotropy and gravity, x y and z contain r theta and phi
  call compute_forces_crust_mantle(ell_d80,minus_gravity_table,density_table,minus_deriv_gravity_table, &
          ...
          alphaval,betaval,gammaval,factor_common,update_dof_crust_mantle,nglob_crust_mantle,index_i,index_k,index_dim)

  call compute_forces_inner_core(minus_gravity_table,density_table,minus_deriv_gravity_table,iter, &
          ...
          alphaval,betaval,gammaval,factor_common,update_dof_inner_core,index_i,index_k,index_dim)

!CDIR NOVECTOR
  do isource = 1,NSOURCES

!   add the source (only if this proc carries the source)
!   only if iter=1 (works fine since source can never be in matching layer)
    if(myrank == islice_selected_source(isource) .and. iter == 1) then
...
! but for all iterations by definition
...
! compute pressure, taking gravity into account
          if(GRAVITY_VAL) then
            pressure = RHO_TOP_OC * (- veloc_outer_core(iglob) &
...
! compute pressure, taking gravity into account
          if(GRAVITY_VAL) then
            pressure = RHO_BOTTOM_OC * (- veloc_outer_core(iglob) &
...
! crust and mantle
  call assemble_MPI_vector(myrank,iter,accel_crust_mantle,nglob_crust_mantle, &
            iproc_xi,iproc_eta,ichunk,addressing, &
            iboolleft_xi_crust_mantle,iboolright_xi_crust_mantle,iboolleft_eta_crust_mantle,iboolright_eta_crust_mantle, &
            npoin2D_faces_crust_mantle,npoin2D_xi_crust_mantle,npoin2D_eta_crust_mantle, &
            iboolfaces_crust_mantle,iboolcorner_crust_mantle, &
...
! inner core
  call assemble_MPI_vector(myrank,iter,accel_inner_core,NGLOB_INNER_CORE, &
            iproc_xi,iproc_eta,ichunk,addressing, &
            iboolleft_xi_inner_core,iboolright_xi_inner_core,iboolleft_eta_inner_core,iboolright_eta_inner_core, &
            npoin2D_faces_inner_core,npoin2D_xi_inner_core,npoin2D_eta_inner_core, &
            iboolfaces_inner_core,iboolcorner_inner_core, &
...
! only at first iteration because no coupling region in the central cube
  if(INCLUDE_CENTRAL_CUBE .and. iter==1) then
...
! send buffers to slices
  ...
  if(ROTATION_VAL) then
    do i=1,nglob_crust_mantle
      if(iter == 1 .or. update_dof_crust_mantle(i)) then
...
!CDIR NOASSUME
  do i=1,nglob_crust_mantle_ndim
    veloc_crust_mantle(i,1) = vpred_crust_mantle(i,1) + deltatover2*accel_crust_mantle(i,1)
  enddo

  if(ROTATION_VAL) then
    do i=1,NGLOB_INNER_CORE
      if(iter == 1 .or. update_dof_inner_core(i)) then
...
!CDIR NOASSUME
  do i=1,NGLOB_INNER_CORE*NDIM
    veloc_inner_core(i,1) = vpred_inner_core(i,1) + deltatover2*accel_inner_core(i,1)
  enddo
...
! compute internal forces in the fluid region
  ...
  call compute_forces_outer_core(time,deltat,two_omega_earth, &
         A_array_rotation,B_array_rotation, &
         minus_rho_g_over_kappa_fluid,iter,displ_outer_core,accel_outer_core, &
         xstore_outer_core,ystore_outer_core,zstore_outer_core, &
         xix_outer_core,xiy_outer_core,xiz_outer_core, &
         etax_outer_core,etay_outer_core,etaz_outer_core, &
         ...
         hprimewgll_xx,hprimewgll_yy,hprimewgll_zz, &
         wgll_cube,wgllwgll_yz_no_i,wgllwgll_xz_no_j,wgllwgll_xy_no_k, &
         ibool_outer_core,idoubling_outer_core,nspec_outer_core, &
         nglob_outer_core,update_dof_outer_core,index_fluid_i,index_fluid_k)
...
! corresponding points are located at the bottom of the mantle
          ...
          vx = veloc_inner_core(1,iglob)
          vy = veloc_inner_core(2,iglob)
          vz = veloc_inner_core(3,iglob)
...
! outer core
  call assemble_MPI_scalar(myrank,iter,accel_outer_core,nglob_outer_core, &
            iproc_xi,iproc_eta,ichunk,addressing, &
            iboolleft_xi_outer_core,iboolright_xi_outer_core,iboolleft_eta_outer_core,iboolright_eta_outer_core, &
            npoin2D_faces_outer_core,npoin2D_xi_outer_core,npoin2D_eta_outer_core, &
            iboolfaces_outer_core,iboolcorner_outer_core, &
...
! and update velocity (this done for all DOFs even if iter > 1)

  do i=1,nglob_outer_core
    if(iter == 1 .or. update_dof_outer_core(i)) &
      accel_outer_core(i) = accel_outer_core(i)*rmass_outer_core(i)
    veloc_outer_core(i) = vpred_outer_core(i) + deltatover2*accel_outer_core(i)
  enddo

! end of iteration loop for matching
  enddo
