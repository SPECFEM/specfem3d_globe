...
!----
  ...
  subroutine assemble_MPI_scalar(myrank,iter,array_val,nglob, &
...
!CDIR NODEP(array_val)
  do ipoin=1,npoin2D_xi
  if(iter==1 .or. icode_xi(ipoin)==IFLAG_MATCHING_POINT) &
...
!CDIR NODEP(array_val)
  do ipoin=1,npoin2D_eta
  if(iter==1 .or. icode_eta(ipoin)==IFLAG_MATCHING_POINT) &
...
!CDIR NODEP(array_val)
  ...
  if(iter==1 .or. icodefaces(ipoin2D,icount_faces)==IFLAG_MATCHING_POINT) &
...
!CDIR NODEP(array_val)
    ...
    if(iter==1 .or. icodecorner(ipoin1D,icount_corners)==IFLAG_MATCHING_POINT) &
