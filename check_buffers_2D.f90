...
! 2-D addressing and buffers for summation between slices
  integer, dimension(:), allocatable :: iboolleft_xi,iboolright_xi, &
    iboolleft_eta,iboolright_eta,icodeleft_xi,icoderight_xi,icodeleft_eta,icoderight_eta
...
! dynamic memory allocation for arrays
  ...
  allocate(icodeleft_xi(NPOIN2DMAX_XMIN_XMAX(iregion_code)))
  allocate(icoderight_xi(NPOIN2DMAX_XMIN_XMAX(iregion_code)))
  allocate(icodeleft_eta(NPOIN2DMAX_YMIN_YMAX(iregion_code)))
  allocate(icoderight_eta(NPOIN2DMAX_YMIN_YMAX(iregion_code)))
...
! read iboolright_xi of this slice
  ...
  npoin2D_xi = 1
 360  continue
  read(34,*) iboolright_xi(npoin2D_xi),icoderight_xi(npoin2D_xi), &
...
! read iboolleft_xi of other slice
  ...
  npoin2D_xi = 1
 350  continue
  read(34,*) iboolleft_xi(npoin2D_xi),icodeleft_xi(npoin2D_xi), &
...
! to see if it is correctly sorted
  do ipoin = 1,npoin2D_xi
      ...
      if(icodeleft_xi(ipoin) /= icoderight_xi(ipoin)) then
         print *,'error: different codes xi ',icodeleft_xi(ipoin),icoderight_xi(ipoin),' in region ',iregion_code
         stop 'error: different codes'
      endif
  enddo
...
! read iboolright_eta of this slice
  ...
  npoin2D_eta = 1
 460  continue
  read(34,*) iboolright_eta(npoin2D_eta),icoderight_eta(npoin2D_eta), &
...
! read iboolleft_eta of other slice
  ...
  npoin2D_eta = 1
 450  continue
  read(34,*) iboolleft_eta(npoin2D_eta),icodeleft_eta(npoin2D_eta), &
...
! to see if it is correctly sorted
  do ipoin = 1,npoin2D_eta
      ...
      if(icodeleft_eta(ipoin) /= icoderight_eta(ipoin)) then
         print *,'error: different codes eta ',icodeleft_eta(ipoin),icoderight_eta(ipoin),' in region ',iregion_code
         stop 'error: different codes'
      endif
  enddo
...
! deallocate arrays
  ...
  deallocate(icodeleft_xi)
  deallocate(icoderight_xi)
  deallocate(icodeleft_eta)
  deallocate(icoderight_eta)
  deallocate(xleft_xi)
  deallocate(yleft_xi)
  deallocate(zleft_xi)
