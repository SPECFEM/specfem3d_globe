...
program adj_traveltime
  ...
  ! adjoint station
  character(len=10),parameter :: station='ZKR'
  character(len=10),parameter :: network='GE'
  ...
  real, parameter :: freq_low(1) = 6.67d-3     ! 1 / 150s
  real, parameter :: freq_high(1) = 3.33d-2    ! 1 / 30s
  ...
  ! trace
  file_data = './SEM/'//trim(network)//'.'//trim(station)//'.'//comp//'.sem.ascii'
  ...
  do itime = 1,nstep
    t_trace(itime) = t_trace(itime) - t_trace(1)
  enddo
  ...
  ! 1st contribution
  ! vertical component
  file_data = './SEM/'//trim(network)//'.'//trim(station)//'.'//comp//'.adj'
  ...
  ! 2nd contribution
  ! time-reversed
  file_data = './SEM/'//trim(network)//'.'//trim(station)//'.'//comp//'.adj_reversed'
  open(unit=1002,file=trim(file_data),status='unknown',iostat=ier)
  if (ier /= 0) stop 'Error opening output adjoint file in SEM/ '
  do itime = 1,nstep
  ...
  ! east component - zeroed out
  file_data = './SEM/'//trim(network)//'.'//trim(station)//'.MXE.adj'
  open(unit=1002,file=trim(file_data),status='unknown',iostat=ier)
  if (ier /= 0) stop 'Error opening output adjoint file in SEM/ '
  do itime = 1,nstep
    write(1002,*) t_trace(itime), 0.0
  enddo
  close(1002)

  ! north component - zeroed out
  file_data = './SEM/'//trim(network)//'.'//trim(station)//'.MXN.adj'
  open(unit=1002,file=trim(file_data),status='unknown',iostat=ier)
  if (ier /= 0) stop 'Error opening output adjoint file in SEM/ '
  do itime = 1,nstep
    write(1002,*) t_trace(itime), 0.0
  ...
  print *,'  .adj          -- adjoint source file containing 1st contribution'
  print *,'  .adj_reversed -- adjoint source file containing time-reversed, 2nd contribution'
  print *
