...
#
# note: not zip format, direct data file - case not handled yet
...
def get_url(topo):
    ...
    if topo == 'etopo1':
        ...
        print("Invalid topo argument %s, only recognizes etopo1,etopo2,etopo4,etopo15" % topo)
        sys.exit(1)
    return url
        ...
        with open('topo.zip', 'wb') as f:
            if length == 0:
                ...
                for data in r.iter_content(chunk_size=4096):
                    ...
                    bandwidth = incr / 1024. / 1024. / t_elapsed
                    # status bar
                    sys.stdout.write("\r[%s%s] %4.2f MB/s" % ('=' * done, ' ' * (100-done), bandwidth ) )
                    sys.stdout.flush()
...
def resample_topo_file(topo,filename):
    """
    re-samples etopo2 to a coarser grid format
    """
    # re-sampling
    if topo == 'etopo4' or topo == 'etopo15':
        print("resampling")
        ...
        if os.path.isfile(filename):
            ...
            # sampling interval
            if topo == 'etopo4' : Ival = 4   # to 4 minute downsampling
            if topo == 'etopo15': Ival = 15  # to 15 minute downsampling

            print("  downsampling to {}m".format(Ival))
            interval = "-I{}m".format(Ival)
            gridfile = "-GETOPO{}.grd=ns".format(Ival)
            cmd = 'grdsample ETOPO2v2c_f4.nc -Rd ' + gridfile + ' ' + interval
            print("> ",cmd)
...
def smooth_topo_bathy(topo,filename_in,filename_out,SIZE_FILTER_ONE_SIDE=1):
    """
    ...
    with open(filename_out,'wb') as f:
        ...
        for iy in np.arange(NY_BATHY):
            for ix in np.arange(NX_BATHY):
                f.write(ibathy_topo[ix,iy])
    print("")

    return ibathy_topo

def create_PPM_image(ibathy_topo,filename='image_topo_bathy.pnm'):
    """
    creates a gray image of the given topo/bathy array
    """
    ...
    for ix in np.arange(xdim):
        for iy in np.arange(ydim):
            ibathy_topo[ix,iy] = 255 * (ibathy_topo[ix,iy] - min_val) / (max_val - min_val)
...
def convert_ascii2bin_specfem_format(ibathy_topo,filename_out):
    """
...
def create_topo_bathy(topo):
    ...
    """
    print("topo/bathy: %s" % topo)
    print("")

    # sets filenames
    if topo == 'etopo1':
        filename = 'ETOPO1_Ice_g_geotiff.tif'
        filename_out = 'topo_bathy_etopo1_ice_c_original_unmodified_unsmoothed.dat'
        filename_out_smoothed = 'topo_bathy_etopo1_smoothed_window_1.dat'
        SIZE_FILTER_ONE_SIDE = 1

    elif topo == 'etopo2':
        filename = 'ETOPO2v2c_f4.nc'
        filename_out = 'topo_bathy_etopo2v2c_original_unmodified_unsmoothed.dat'
        filename_out_smoothed = 'topo_bathy_etopo2v2c_smoothed_window_3.dat'
        SIZE_FILTER_ONE_SIDE = 3

    elif topo == 'etopo4':
        filename_org = 'ETOPO2v2c_f4.nc'
        filename = 'ETOPO4.grd'
        filename_out = 'topo_bathy_etopo4_unmodified_unsmoothed.dat'
        filename_out_smoothed = 'topo_bathy_etopo4_smoothed_window_7.dat'
        SIZE_FILTER_ONE_SIDE = 7

    elif topo == 'etopo15':
        filename_org = 'ETOPO2v2c_f4.nc'
        filename = 'ETOPO15.grd'
        filename_out = 'topo_bathy_etopo15_unmodified_unsmoothed.dat'
        filename_out_smoothed = 'topo_bathy_etopo15_smoothed_window_5.dat'
        SIZE_FILTER_ONE_SIDE = 3
    ...
    #filename = 'etopo1_ice_c_resampled_at_2minutes.grd'
    #filename_out = 'topo_bathy_etopo1_ice_c_resampled_at_2minutes_original_unmodified_unsmoothed.dat'

    # data file download
    if topo == 'etopo4' or topo == 'etopo15':
        # web download
        download_data_file(topo,filename_org)
        # re-samples topo
        resample_topo_file(topo,filename)
    else:
        # web download
        download_data_file(topo,filename)

    # extracts elevation
    extract_to_ascii(topo,filename,filename_out)

    # smoothing
    ibathy_topo = smooth_topo_bathy(topo,filename_out,filename_out_smoothed,SIZE_FILTER_ONE_SIDE)

    # PPM image
    create_PPM_image(ibathy_topo)
    ...
    convert_ascii2bin_specfem_format(ibathy_topo,filename_out_smoothed + '.bin')
...
if __name__ == '__main__':
    ...
    if '--help' in sys.argv or '-h' in sys.argv or len(sys.argv) != 2:
        print("usage: ./create_topo_bathy_file.py etopo1 [or etopo2,etopo4,etopo15]")
        sys.exit(1)
    else:
        topo = sys.argv[1]
